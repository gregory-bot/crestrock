...
const Category: React.FC<CategoryProps> = ({ searchQuery, onSearchChange }) => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const products = await FirebaseService.getProductsByBrand(brand);
      setItems(products);
    } catch (error) {
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {items.map((p) => (
              <div
                key={p.id}
                className="category-card bg-white rounded-lg shadow-md overflow-hidden transform transition duration-300 hover:-translate-y-2 hover:shadow-lg motion-reduce:transform-none border border-gray-100"
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-lg font-bold text-blue-600">KSH {p.price.toLocaleString()}</div>
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mb-4">Try another category or return to <Link to="/" className="text-blue-600 hover:underline">home</Link>.</p>
          </div>
        )}
      </div>
